"""
PYTHONPATH=.. python -m evaluations.evaluations_single_phase_absolute_time seed0_1

Same report as evaluations_single_phase.py, but for config_single_phase_absolute_time runs --
...
plain-RBF baseline model with `time` KEPT as a GP regressor. Regenerates every plot/table from just the run id
(e.g. "seed0_1", resolved under results/single_phase_absolute_time/) -- everything else is read
...
Only load_run/reconstruct_gp_agent/plot_fs_residual differ from evaluations_single_phase.py:
  ...
  * load_run/reconstruct_gp_agent are passed config_single_phase_absolute_time.get_config and this
...
from mcpilco.config_single_phase_absolute_time import get_config as absolute_get_config

ABSOLUTE_RESULTS_ROOT = Path(_ROOT) / "results" / "single_phase_absolute_time"
...
if __name__ == "__main__":
    ...
    p.add_argument("run_id", type=str,
                   help="run to evaluate, e.g. 'seed0_1' (resolved under "
                        "results/single_phase_absolute_time/) or a full/relative path to a run folder")
    ...
    p.add_argument("--results_root", type=str, default=None,
                   help="override the results root run_id is resolved under "
                        "(default: results/single_phase_absolute_time/)")
    args = p.parse_args()
