...
from mcpilco.config_dual_phase_baseline_time import get_config as baseline_get_config

BASELINE_RESULTS_ROOT = Path(_ROOT) / "results" / "dual_phase_baseline_time"
...
if __name__ == "__main__":
    ...
    p.add_argument("run_id", type=str,
                   help="run to evaluate, e.g. 'seed1_1' (resolved under "
                        "results/dual_phase_baseline_time/) or a full/relative path to a run folder")
    ...
    p.add_argument("--results_root", type=str, default=None,
                   help="override the results root run_id is resolved under "
                        "(default: results/dual_phase_baseline_time/)")
    args = p.parse_args()
