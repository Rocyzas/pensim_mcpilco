"""
PYTHONPATH=.. python -m evaluations.evaluations_single_phase seed3_104

Regenerates every plot/table for a single-phase MC-PILCO run from just its run id (e.g.
"seed3_104", resolved under results/single_phase/) -- everything else (SEED, NUM_TRIALS, FAST,
optim_horizon/anchors, the trained policy, the trained GPs) is read back from that run's own
note.txt/log.pkl/monitor.pkl. All output (PNG/CSV) is written flat into the run's own folder.

Same analysis as evaluations.ipynb -- both import from eval_single_phase_lib, so there is
exactly one implementation of every plot/table.
"""
import argparse
...
def main(run_id, gp_trial=None, n_eval_seeds=5, eval_base=700000, compare_seed=None):
    ...
    lib.plot_particle_bands(gp_agent, gp_idx, ho_idx, has_ho, out_dir, show=False)
