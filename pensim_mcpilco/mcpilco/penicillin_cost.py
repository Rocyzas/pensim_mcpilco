...
class PeniConcentrationCost(CF.Expected_cost):
    ...
    # NOTE: two knobs were tried here and REMOVED, both dead ends (see SUPERVISOR_REVIEW.md):
    #   action_penalty (anchor the action to the recipe, penalise |a|) -- did not stop the flooring;
    #   beta_cost_std  (pessimism: optimise mean + beta*std over particles, via a forward() override)
    #     -- did nothing. The cost already prefers the recipe: sweeping a HELD action, its optimum is
    #     a=0 for every p_weight tried, and the feed floor is the WORST option.
    # Keeping stock Expected_cost.forward (no override).
