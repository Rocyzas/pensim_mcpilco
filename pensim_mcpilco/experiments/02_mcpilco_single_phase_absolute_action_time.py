"""
 PYTHONPATH=.. python -m experiments.02_mcpilco_single_phase_absolute_action_time --seed 1 --num_trials 5 --fast

ABSOLUTE-ACTION experiment WITH `time` kept as a GP regressor: the `time`-carrying twin of
02_mcpilco_single_phase_absolute_action.py, built on config_single_phase_absolute_time. It stands
to 02_mcpilco_single_phase_baseline_time.py exactly as that driver stands to
02_mcpilco_single_phase_baseline.py -- same plain-RBF model, same absolute action encoding, the
only difference from its no-time twin being that `time` is NOT dropped from any GP's active_dims.
Run the pair to separate the action-encoding effect from the `time`-regressor effect. The policy
...
three arms of the residual/absolute x time/no-time grid.
...
_RESULTS_ROOT = Path(_ROOT) / "results" / "single_phase_absolute_time"
...
def main(seed=1, num_trials=10, fast=False, out_dir=None,
    ...
    # config_single_phase_absolute_time (and everything it transitively imports) is EVER imported
    ...
    from mcpilco.config_single_phase_absolute_time import get_config
