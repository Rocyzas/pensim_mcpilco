"""
 PYTHONPATH=.. python -m experiments.02_mcpilco_single_phase_baseline --seed 1 --num_trials 5 --fast

Plain-RBF ablation baseline: identical driver to 02_mcpilco_single_phase.py, but built on
config_single_phase_baseline (Wt mass-balance / Viscosity recipe-mean prior means removed --
every channel uses a vanilla RBF, see model_learning_baseline.py) and logging to its own
results/single_phase_baseline/ tree so it never collides with config_single_phase runs.
"""
import argparse
import datetime
import pickle
...
_RESULTS_ROOT = Path(_ROOT) / "results" / "single_phase_baseline"
...
def main(seed=1, num_trials=10, fast=False, out_dir=None,
    ...
    # config_single_phase_baseline (and everything it transitively imports) is EVER imported in
